...
/** Default store logs the message and acknowledges it. */
SqsSink.prototype._store = null


/**
 * Sets a message store for handling messages.
 * @param {Object} store
 * @return {SqsSink}
 */
SqsSink.prototype.setStore = function (store) {
  if (store) {
    store.sink = this
    this._store = store
  }
 ...
 */
SqsSink.prototype.receive = function () {
...
SqsSink.prototype._onReceive = function (err, data) {
  if (err) {
    ...
    data['Messages'].forEach(function (message) {
      messages[message['ReceiptHandle']] = message['Body']
    })

    this._emptyPollDelaySec = 0
    this._store.handleMessages(messages, this._wrap(this._receiveInternal))
  }
}
