 ...
 */
function MessageStore(sink, outDir, filenamePattern, flushFreqSec, udpPort, udpHost) {
...
MessageStore.prototype._sendToUdp = function(data, port, host, callback) {
  message = new Buffer(data, 'utf8')
  client.send(message, 0, message.length, port, host, function(err, bytes) {
    if (err) {
      this.logger.error('Error writing to UDP', messages, err.stack)
    }
  })
};
...
MessageStore.prototype._flush = function () {
  ...
  this.write(data, function (err, identifier) {
    if (err) {
      ...
      if (this._udpPort) {
        this._sendToUdp(data, this._udpPort, this._udpHost)
      }
